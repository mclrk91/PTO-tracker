    ...
    <div className="space-y-4">
        ...
        <div>
          ...
          <div className="space-y-2">
            {holidayOpps.map((opp, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center justify-between card-hover">
                <div className="flex-1">
                  <p className="text-sm font-medium text-slate-700">{opp.description}</p>
          ...
          <div className="space-y-2">
            {flexOpps.slice(0, 8).map((opp, i) => (
              <div key={i} className="bg-white rounded-xl border border-slate-200 p-3 flex items-center justify-between card-hover">
                <div className="flex-1">
                  <p className="text-sm font-medium text-slate-700">{opp.description}</p>
                  <span className="text-xs text-primary-600">{opp.ptoDays} PTO day ({opp.ptoHours}h)</span>
                </div>
                <button onClick={() => handleAddToPlanner(opp)}
                  className="ml-3 text-xs px-3 py-1.5 bg-primary-100 text-primary-700 rounded-lg hover:bg-primary-200 transition-colors whitespace-nowrap">
                  Add to Planner
      ...
      {opportunities.length === 0 && (
        <div className="bg-white rounded-xl border border-slate-200 p-8 text-center">
          ...
          <p className="text-sm text-slate-500">No long weekend opportunities found for the rest of the year.</p>
        </div>
      )}
    </div>
  );
